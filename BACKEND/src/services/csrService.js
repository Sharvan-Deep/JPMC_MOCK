const { Company, CSRPolicy, Source } = require('../models');
...
async function getCsrOverview(companyId) {
  ...
  const [policyCount, policyYears, sourceCount, recentPolicies] = await Promise.all([
  ...
  const availableFinancialYears = [
    ...new Set([...(company.financial_years || []), ...policyYears.filter(Boolean)]),
  ].sort();
